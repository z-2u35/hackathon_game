...
export default function UserPage() {
  ...
  const lanternId = hasLantern ? lanternData?.data?.[0]?.data?.objectId ?? null : null;

  if (!account) return null; // hoặc redirect về /auth bằng AuthProvider
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-zinc-950 text-white">
      <Disclosure defaultOpen>
        {({ }) => (
          <>
            <Disclosure.Button className="px-6 py-3 mb-4 bg-amber-700/30 rounded-md font-pixel border border-amber-500 hover:bg-amber-600/50 transition-all">
            ...
            <Disclosure.Panel className="flex flex-col items-center gap-6">
              {hasLantern && lanternId ? (
                ...
                <p className="text-zinc-400 mt-4">Bạn chưa có nhân vật, hãy mint để bắt đầu.</p>
              )}
            </Disclosure.Panel>
          </>
        )}
